...
import { Box, Typography, Container, Paper, Button, Card, CardContent, CircularProgress, FormControl, InputLabel, Select, MenuItem, Grid, Snackbar, Alert } from '@mui/material';
...
const ScanDocument = () => {
  ...
  const handleSaveDocument = async () => {
    if (!scanResults || !currentUser) return;
    
    try {
      setSaving(true);
      
      // Create a document object with the scan results
      const documentData = {
        title: `Scanned ${documentType.charAt(0).toUpperCase() + documentType.slice(1)} Document`,
        content: scanResults.extractedText || 'No text extracted',
        documentType: documentType,
        tags: [documentType, 'scanned'],
        metadata: {
          scanConfidence: scanResults.confidence,
          scanTimestamp: scanResults.processingTimestamp,
          extractedData: scanResults.extractedData
        },
        fileType: 'text/plain'
      };
      
      // Save the document using the API
      ...
      // Optional: Navigate to the document viewer
      ...
      setNotification({
        open: true,
        message: `Error saving document: ${err.message || 'Unknown error'}`,
        severity: 'error'
      });
    } finally {
      setSaving(false);
