...
from utils.auth import generate_verification_token, verify_token, generate_tokens
...
def get_users_collection() -> Optional[Collection]:
    ...
    try:
        db: Optional[Database] = getattr(ext_mongo, 'db', None)
        if db is None:
...
def register():
    try:
        ...
        additional_claims = {'roles': data['role']}
...
def logout():
    try:
        ...
        if 'jwt_blocklist' in current_app.extensions:
            current_app.extensions['jwt_blocklist'].add(jti)
        else:
            # Fallback to module-level blocklist if extension is not available
...
def test_admin():
    ...
    try:
        ...
        logger.error(f"Test admin error: {str(e)}")
        return jsonify({'error': 'Admin authentication test failed'}), 500
