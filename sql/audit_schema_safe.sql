-- Safe Audit System Database Schema for SmartProBono
-- This version handles existing types and tables gracefully

-- Create audit event types enum (only if it doesn't exist)
DO $$ BEGIN
...
-- Create audit severity enum (only if it doesn't exist)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS api_audits (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_api_audits_endpoint ON api_audits(endpoint);
CREATE INDEX IF NOT EXISTS idx_api_audits_user_id ON api_audits(user_id);
CREATE INDEX IF NOT EXISTS idx_api_audits_created_at ON api_audits(created_at);

CREATE INDEX IF NOT EXISTS idx_document_audits_document_id ON document_audits(document_id);
CREATE INDEX IF NOT EXISTS idx_document_audits_user_id ON document_audits(user_id);
CREATE INDEX IF NOT EXISTS idx_document_audits_action ON document_audits(action);
...
DROP POLICY IF EXISTS "System can insert user activities" ON user_activities;
...
DROP POLICY IF EXISTS "Users can view their own compliance records" ON compliance_records;
DROP POLICY IF EXISTS "Admins can view all compliance records" ON compliance_records;
DROP POLICY IF EXISTS "System can insert compliance records" ON compliance_records;
DROP POLICY IF EXISTS "Admins can view all API audits" ON api_audits;
DROP POLICY IF EXISTS "System can insert API audits" ON api_audits;
DROP POLICY IF EXISTS "Users can view their own document audits" ON document_audits;
DROP POLICY IF EXISTS "Admins can view all document audits" ON document_audits;
DROP POLICY IF EXISTS "System can insert document audits" ON document_audits;
