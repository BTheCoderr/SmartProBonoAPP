  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.text(
          `Generated by SmartProBono Legal Assistant - Page ${i} of ${pageCount}`,
          20,
          290
        );
      }
  ...
  const detectTaskType = (message) => {
    ...
    const patterns = {
      document_drafting: {
        ...
        documents: ['contract', 'agreement', 'document', 'form', 'letter', 'template', 'nda', 'lease'],
        weight: 0
      },
      rights_research: {
    ...
    for (const [task, pattern] of Object.entries(patterns)) {
      // Check for keywords
      if (pattern.keywords) {
        pattern.weight += pattern.keywords.filter(word => 
      ...
      if (pattern.documents) {
        pattern.weight += pattern.documents.filter(doc => 
      ...
      // Check for context
      if (pattern.contexts) {
        pattern.weight += pattern.contexts.filter(context => 
    ...
    const sortedTasks = Object.entries(patterns)
    ...
    <Container maxWidth="lg">
      ...
      {error && (
        ...
        <List>
          ...
          <TaskSpecificComponents taskType={taskType} response={messages[messages.length - 1]?.text} />
      ...
      <Dialog open={pdfDialogOpen} onClose={() => setPdfDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Your {taskType.replace(/_/g, ' ')} will be formatted into a PDF document that you can download and save for your records.
          </Typography>
        </DialogContent>
        <DialogActions>
          <span>
